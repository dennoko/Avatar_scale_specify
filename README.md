...
2. Unityを再起動またはスクリプトリロードして、メニュー `dennokoworks/ViewPoint Scaler` が追加されたことを確認します。

## 使い方
1. スケール調整したいアバターのルート（`VRCAvatarDescriptor` と同じ階層）を選択し、`dennokoworks/ViewPoint Scaler` メニューから `ViewPointScaler` を追加。もしくは `GameObject/dennokoworks/Add ViewPoint Scaler Child` で子オブジェクト `AvatarScaler` を生成し、その子にコンポーネントを付与すると、設定を別アバターへ複製しやすくなります。
